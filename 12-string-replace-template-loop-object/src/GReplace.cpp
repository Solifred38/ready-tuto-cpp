...
GReplace GReplace::render() {
    ...
    lReplace.renderInternal();
    m_errors.addErrors(lReplace.getErrors());
    return lReplace;
}
//===============================================
void GReplace::renderInternal() {
    //runForInternal();
    runForObjectInternal();
    runDataInternal();
}
//===============================================
...
//===============================================
void GReplace::runForObjectInternal() {
    int lCount = m_replaceText.countMatch("\\{\\{\\w+([\\.\\s]?\\,?\\s?\\w+)*\\}\\}");
    int lCountFor = 0;
    int lCurPos = 0;
    int lCurPatternSize = 0;
    int lForPos = 0;
    ...
    for(int i = 0; i < lCount; i++) {
        GString lPattern = m_replaceText.searchMatch("\\{\\{(\\w+([\\.\\s]?\\,?\\s?\\w+)*)\\}\\}", i, 0);
        GString lName = m_replaceText.searchMatch("\\{\\{(\\w+([\\.\\s]?\\,?\\s?\\w+)*)\\}\\}", i, 1);
        m_patternMap[lName] = lPattern;
        lCurPos = m_replaceText.indexOf(lPattern, lCurPos);
        lCurPatternSize = lPattern.size();
        ...
        if(!lForPatternText.empty() && !lForText.empty()) {
            GString lForPattern = lForPatternText.searchMatch("(\\w+) , (\\w+) in (\\w+)", 0, 0);
            GString lForKey     = lForPatternText.searchMatch("(\\w+) , (\\w+) in (\\w+)", 0, 1);
            GString lForValue   = lForPatternText.searchMatch("(\\w+) , (\\w+) in (\\w+)", 0, 2);
            GString lForMap     = lForPatternText.searchMatch("(\\w+) , (\\w+) in (\\w+)", 0, 3);

            GReplace lForReplace;
            lForReplace.setReplaceMap(m_replaceMap);
            lForReplace.setReplaceText(lForText);
            lForReplace.setForKeyName(lForKey);
            lForReplace.setForValueName(lForValue);
            lForReplace.setForMapName(lForMap);
            lForReplace.renderForObjectInternal();
...
void GReplace::renderForInternal() {
    ...
    for(it = m_replaceMap.begin(); it != m_replaceMap.end(); it++) {
        ...
        GString lKeyName = m_forMapName + ".\\d+";
        if(lKey.isMatch(lKeyName)) {
            GReplace lReplace = *this;
...
//===============================================
void GReplace::renderForObjectInternal() {
    std::map<GString, GString>::iterator it;
    GString lForText;
    int lIndex = 0;
    int lIndex1 = 0;
    for(it = m_replaceMap.begin(); it != m_replaceMap.end(); it++) {
        GString lKey = it->first;
        GString lValue = it->second;
        GString lKeyName = m_forMapName + ".[\\w\\d]+";
        if(lKey.isMatch(lKeyName)) {
            GReplace lReplace = *this;
            lReplace.addReplaceData(m_forKeyName, lKey);
            lReplace.addReplaceData(m_forValueName, lValue);
            lReplace.addReplaceData("loop.index", lIndex++);
            lReplace.addReplaceData("loop.index1", ++lIndex1);
            lReplace.runDataInternal();
            lForText += lReplace.getReplaceText();
